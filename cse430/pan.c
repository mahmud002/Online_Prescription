...
short src_ln0 [] = {
	  0,  16,  18,  20,  21,  22,  24,  25, 
	 26,  27,  28,  23,  30,  19,  31,  31, 
	 32,  17,  34,  17,  35,  36,  37,  34, 
	 39,  39,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "profile.pml", 1, 25 },
...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* sieve */
		...
		/* params: */
		((P0 *)pptr(h))->c = par0;
		((P0 *)pptr(h))->prime = par1;
		/* locals: */
		((P0 *)pptr(h))->child = addqueue(calling_pid, 1, 1);
		((P0 *)pptr(h))->haschild = 0;
		((P0 *)pptr(h))->n = 0;
#ifdef VAR_RANGES
		logval("sieve:haschild", ((P0 *)pptr(h))->haschild);
		logval("sieve:prime", ((P0 *)pptr(h))->prime);
		logval("sieve:n", ((P0 *)pptr(h))->n);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
{
		now.root = addqueue(calling_pid, 2, 1);
#ifdef VAR_RANGES
#endif
}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.root) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.root) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.root) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.root) )))
			    ) return 0; break;
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P0 *)_this)->c) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P0 *)_this)->c) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P0 *)_this)->c) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P0 *)_this)->c) )))
			    ) return 0; break;
		case 3: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( ((P0 *)_this)->child) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( ((P0 *)_this)->child) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( ((P0 *)_this)->child) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( ((P0 *)_this)->child) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  number:	2\n");
	printf("	mtype  eof:	1\n");
	printf("	chan root (=%d):	len %d:\t", now.root, q_len(now.root));
	c_chandump(now.root);
}
void
...
{	/* int i; */
	...
	printf("	int    n:	%d\n", ((P1 *)pptr(pid))->n);
		break;
	case 0:
		printf("local vars proc %d (sieve):\n", pid);
	printf("	chan c (=%d):	len %d:\t", ((P0 *)pptr(pid))->c, q_len(((P0 *)pptr(pid))->c));
	c_chandump(((P0 *)pptr(pid))->c);
	printf("	int    prime:	%d\n", ((P0 *)pptr(pid))->prime);
	printf("	chan child (=%d):	len %d:\t", ((P0 *)pptr(pid))->child, q_len(((P0 *)pptr(pid))->child));
	c_chandump(((P0 *)pptr(pid))->child);
	printf("	bit    haschild:	%d\n", ((P0 *)pptr(pid))->haschild);
	printf("	int    n:	%d\n", ((P0 *)pptr(pid))->n);
		break;
	}
}
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("eof"); return;
	case 2: Printf("number"); return;
