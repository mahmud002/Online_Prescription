...
iniglobals(int calling_pid)
{
		...
		now.to_profile = addqueue(calling_pid, 2, 0);
...
#endif
	switch (n) {
	case 3: j = sizeof(Q3); q_flds[3] = 1; q_max[3] = 9; break;
	case 2: j = sizeof(Q2); q_flds[2] = 1; q_max[2] = 15; break;
	case 1: j = sizeof(Q1); q_flds[1] = 1; q_max[1] = 9; break;
...
{	if (!from--)
	...
	switch(((Q0 *)qptr(from))->_t) {
	case 3: return (q_sz(from) == 9);
	case 2: return (q_sz(from) == 15);
	case 1: return (q_sz(from) == 9);
	case 0: printf("queue %d was deleted\n", from+1);
	}
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_profile) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_profile) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_profile) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_profile) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  enterProfile:	12\n");
	printf("	mtype  addinfo:	11\n");
	printf("	mtype  checkAuth:	10\n");
	printf("	mtype  authSuccess:	9\n");
	printf("	mtype  loadProfile:	8\n");
	printf("	mtype  editProfile:	7\n");
	printf("	mtype  returnForm:	6\n");
	printf("	mtype  insertInfo:	5\n");
	printf("	mtype  saveInfo:	4\n");
	printf("	mtype  updateSuccess:	3\n");
	printf("	mtype  ack:	2\n");
	printf("	mtype  ack0:	1\n");
	printf("	chan to_user (=%d):	len %d:\t", now.to_user, q_len(now.to_user));
	c_chandump(now.to_user);
	printf("	chan to_profile (=%d):	len %d:\t", now.to_profile, q_len(now.to_profile));
	c_chandump(now.to_profile);
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (Profile):\n", pid);
		break;
	case 0:
...
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("ack0"); return;
	case 2: Printf("ack"); return;
	case 3: Printf("updateSuccess"); return;
	case 4: Printf("saveInfo"); return;
	case 5: Printf("insertInfo"); return;
	case 6: Printf("returnForm"); return;
	case 7: Printf("editProfile"); return;
	case 8: Printf("loadProfile"); return;
	case 9: Printf("authSuccess"); return;
	case 10: Printf("checkAuth"); return;
	case 11: Printf("addinfo"); return;
	case 12: Printf("enterProfile"); return;
