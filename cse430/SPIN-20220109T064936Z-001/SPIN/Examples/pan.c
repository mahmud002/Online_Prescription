...
/*** From source: newTextDocument.pml ***/
...
iniglobals(int calling_pid)
{
		...
		now.to_signup = addqueue(calling_pid, 2, 0);
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		case 2: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.to_signup) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.to_signup) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.to_signup) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.to_signup) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	mtype  signuph:	7\n");
	printf("	mtype  addinfo:	6\n");
	printf("	mtype  check:	5\n");
	...
	printf("	chan to_signup (=%d):	len %d:\t", now.to_signup, q_len(now.to_signup));
	c_chandump(now.to_signup);
...
{	/* int i; */
	...
	case 2:
		...
		break;
	case 1:
		printf("local vars proc %d (Signup):\n", pid);
		break;
	case 0:
...
printm(int x, char *s)
{
	...
	case 5: Printf("check"); return;
	case 6: Printf("addinfo"); return;
	case 7: Printf("signuph"); return;
